...
1、整体技术选型(包括脚手架和框架、ES6、ES5)
        选用的是vue的脚手架，echarts，element-ui ，moment，vuex，使用ES6的语法，因为es6有表达式、箭头函数、块级作用域的let和const、模块化等这些特点
        选用vue主要是基于分层的目,让彼此的目的分开，而且vue用的就是普通的HTML，通过Vue来整合现有的系统是比较容易的，不需要整体重构。

2、路由搭建、路由挂载的实现权限接口设计
        路由搭建：因为只需要简单的路由而不需要引入整个路由库，可以动态渲染一个页面级的组件，路由允许我们通过不同的 URL 访问不同的内容
